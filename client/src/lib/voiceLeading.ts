...
// ─── Voicing Width by Rhythm Pattern ─────────────────────────────────────────

function applyVoicingWidth(voicing: number[], pattern: ChordRhythmPattern): number[] {
  if (voicing.length < 2) return voicing;
  const sorted = [...voicing].sort((a, b) => a - b);

  if (pattern === "slow") {
    // Wide/open: drop lowest voice an octave
    const result = [sorted[0] - 12, ...sorted.slice(1)];
    while (result[0] < 28) result[0] += 12;
    return result.sort((a, b) => a - b);
  }

  if (pattern === "active") {
    // Tight: compress all voices within one octave of the top note
    const top = sorted[sorted.length - 1];
    const compressed = sorted.map((n) => {
      let note = n;
      while (note < top - 12) note += 12;
      return note;
    });
    return compressed.sort((a, b) => a - b);
  }

  return voicing; // standard: no change
}
...
export function applyVoiceLeadingPass(
  ...
  const pattern = phraseHarmony.chordRhythmPattern ?? "standard";
  ...
  function processOption(opt: ProgressionOption): ProgressionOption {
    ...
    const widthAdjusted = densityAdjusted.map((chord) => ({
      ...chord,
      voicing: applyVoicingWidth(chord.voicing, pattern),
    }));

    const bassRefined = refineBassLine(widthAdjusted, phrase.energyScore);
